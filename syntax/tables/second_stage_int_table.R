...
dsep <- c(" "="Past Coll. Eff.\nd-Sep. P-value", "", no_lead_zero(setNames(round(psem_hlm_int_list_summary$dTable$P.Value,2), 
                                                          str_remove_all(psem_hlm_int_list_summary$dTable$Independ.Claim, 
                                                                         "^CRIME_|_2004.*"))))
r2vals_2s <- c(" "="R2", "", no_lead_zero(setNames(round(psem_hlm_int_list_summary$R2$Marginal[1:4],2), 
                                  str_remove_all(psem_hlm_int_list_summary$R2$Response[1:4], 
                                                 "^CRIME_|_2004.*"))))
...
s2_int_table <- 
  ...
  set_header_labels(Level = "", Predictor = "Predictor", `Homicide /\nGun Assault` = "Homicide /\nGun Assault", `Gun Assault`="Gun Assault", Robbery = "Robbery", Violent = "Violent", Property = "Property") %>%
  merge_h_range(i = c(1,7), j1 = 1, j2 = 6) %>%
  ...
  align(i = c(2:6, 8:26), j = 3:6, align = "center", part = "body") %>%
  padding(i = c(2:6, 8:26), j = 1, padding.left = 20) %>%
  ...
  add_footer_row(top=FALSE, values = "N = 1641 for all models; Standard errors in parentheses", colwidths = 6) %>%
  add_footer_row(top=FALSE, values = "Bolded estimates significant at 95% level; Trigamma R2", colwidths = 6) %>%
