...
phdcn_nc_to_ccahs_tract <- haven::read_sav(icpsr_tract_crosswalk_path) %>% 
  mutate(across(everything(), ~as.character(.))) %>%
  rename(phdcn_nc = link_nc, 
         ccahs_tract = tract)

ccahs_nc_to_ccahs_tract <- haven::read_dta(ccahs_main_path) %>%
  select(ccahs_nc          = V5,
         ccahs_tract       = V6) %>%
...
save(nc_crosswalk, file = "./data/derived/crosswalks/nc_crosswalk.RData")
...
save(il_tract, file = "./data/derived/il_tract.RData")
...
save(tract_to_nc, file = "./data/derived/crosswalks/tract_to_nc.RData")

#----
# Add blocks

il_block_raw <- sf::read_sf(il_block_1990_path) %>%
  st_transform(3435)
...
save(il_block, file = "./data/derived/il_block.RData")

# SSO IDs

ccahs_sso_ids <- haven::read_dta(ccahs_sso_path) %>% 
...
save(ccahs_sso_ids, file = "./data/derived/ccahs_sso_ids.RData")
...
save(complete_crosswalk, file = "./data/derived/crosswalks/complete_crosswalk.RData")
